...
class ProductCategoryController extends Controller
{
	...
	{
		//原生语句查询法
		$users = DB::select('select * from demo_product');
		//从一张表中取出所有行
		//$users = DB::table('demo_product')->get();
		//像访问对象一样访问字段
		//echo $users[1]->name;
		$us = array();
		...
		//dd($users);
		//从一张表中获取一行/一列
		//$user = DB::table('demo_product')->where('name', '电话卡')->first();
		//echo $user->name;
		return view('2', ['us' => $us]);
	}
	public function finish2(Request $request)
	{
		return view('3', ['name' => '呵呵呵哈克双击打开世界级的哈速度很快就']);
	}
	/**
	 * 处理用户提交的调查问卷
	 ...
	 * @return void
	 */
	public function testBasicExample(Request $request)
	{
		//$input = $request->all();
		echo $input = $request->id;
		// $response = $this->json('POST', '/3', ['name' => '学院君']);
	...
	//用户信息删除Request $request
	public function delInfo(Request $request)
	{
		$userid = $request->id;
		$_sql = "delete from user where id=" . $userid;
		$user = DB::select($_sql);
		dump($user);
		// $user =DB::table('user')->where('id',$userid)
	}
}
